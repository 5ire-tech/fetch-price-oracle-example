...
import {IOracleFeed} from "./IOracleFeed.sol";
...
interface IUmbrellaFeeds {
    function getPriceData(
        ...
        external
        view
        returns (uint8 data, uint24 heartbeat, uint32 timestamp, uint128 price);
 ...
 */
contract UmbrellaAdapter is IOracleFeed {
    /**
    ...
    ) public view returns (uint256 price, uint32 timestamp) {
        (, , uint32 feedTimestamp, uint128 feedPrice) = IUmbrellaFeeds(feed)
            .getPriceData(bytes32(feedKey));
